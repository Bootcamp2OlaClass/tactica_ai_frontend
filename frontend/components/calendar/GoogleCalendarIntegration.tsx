...
/**
 * Google Calendar is an optional integration, not the calendar itself --
 * this is the same connect/disconnect UI that used to be the entire
 * Calendar page, now moved under Calendar -> Integrations. Sync behavior
 * (per-task "Add to calendar", idempotent re-sync, no duplicate events) is
 * unchanged -- only its location in the UI moved.
 */
export function GoogleCalendarIntegration() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <p className="mb-6 text-sm text-[#696977] dark:text-[#9797a6]">
        Tactica AI works fully without Google Calendar. Connect it if you also want your task deadlines synced as
      ...
      {status.status === "success" && (
        <div className="rounded-2xl border border-[#dedee9] bg-white p-6 dark:border-[#2d2d38] dark:bg-[#1b1b23]">
          ...
          <p className="mt-3 text-sm text-[#696977] dark:text-[#9797a6]">
