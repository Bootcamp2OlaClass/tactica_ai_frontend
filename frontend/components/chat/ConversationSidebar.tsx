...
export function ConversationSidebar({
  ...
  return (
    <aside className="flex h-full w-full shrink-0 flex-col border-r border-[#dedee9] bg-white sm:w-64">
      <div className="p-3">
      ...
      <nav aria-label="Conversation history" className="flex-1 space-y-1 overflow-y-auto px-2 pb-3">
        {conversations.length === 0 && (
          <p className="px-2 py-4 text-center text-xs text-[#92929e]">No conversations yet.</p>
        )}
          ...
          return (
            <button
              ...
              className={`block w-full rounded-xl px-3 py-2.5 text-left text-sm transition ${
                active ? "bg-[#f6f4ff] text-[#284fc4]" : "text-[#454550] hover:bg-[#f6f4ff]"
              }`}
            >
              <span className="block truncate font-medium">{conversation.title || "New conversation"}</span>
              <span className="mt-0.5 block text-xs text-[#92929e]">
