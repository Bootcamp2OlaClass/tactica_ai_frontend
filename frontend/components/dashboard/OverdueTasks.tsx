        ...
        <ul className="mt-4 space-y-3">
          {tasks.map((task) => (
            <li key={task.id} className="rounded-xl border border-[#ead3ce] bg-white p-4">
              <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">{task.title}</p><p className="mt-1 text-xs text-[#696977]">{task.courseCode} · Due {formatDate(task.dueAt)}</p></div><span className="shrink-0 rounded-full bg-[#fff0ed] px-2 py-1 text-[10px] font-semibold text-[#a83b29]">{task.priority}</span></div>
