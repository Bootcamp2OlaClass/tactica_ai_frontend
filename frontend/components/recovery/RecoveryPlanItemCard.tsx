...
export function RecoveryPlanItemCard({ item, courseLabel }: { item: RecoveryPlanItem; courseLabel: string | null }) {
  return (
    <div className="rounded-xl border border-[#dedee9] bg-white p-4">
      ...
      <p className="mt-2 text-sm font-medium text-[#17171c]">{item.title}</p>
      ...
      {item.explanation && (
        <div className="mt-3 rounded-lg bg-[#f6f4ff] px-3 py-2">
          <span className="text-xs font-semibold text-[#315bd8]">Coach note: </span>
          <span className="text-sm text-[#454550]">{item.explanation}</span>
        </div>
      )}
    </div>
  );
